...
/**
 * When a tag has a multiline description and/or multiline name, this function will take
 * care of rendering the rest of lines, respecting each property column space and add the
 * necessary padding.
 *
 * @param {number}   column            The column where the lines should start.
 * @param {boolean}  hasName           Whether or not there was a valid `name` property.
 *                                     Based on that, the function will decide if the
 *                                     space for the 'name column' should be padding or if
...
const renderRest = (column, hasName, nameColumnWidth, nameLines, descriptionLines) => {
  ...
  for (let i = 0; i < limit; i++) {
    const nameLine = nameLines[i] ? nameLines[i].padEnd(nameColumnWidth) : namePadding;
    const descriptionLine = descriptionLines[i] ? descriptionLines[i] : '';
...
/**
 * Renders a JSDoc tag using the `columns` style: there's a column for the tag, the type,
 * the name and the description, and if the description is longer than the available
 * space, it will be splitted in multiple lines, with padding for the other columns on
 * each new line.
 *
 * @callback RenderTagInColumnsFn
 * @param {number}     tagColumnWidth          The width of the column for the tag,
 *                                             including the `@` symbol.
 * @param {number}     typeColumnWidth         The width of the column for the type, it
 *                                             will only be used if the tag has a `type`
 *                                             property.
 * @param {number}     nameColumnWidth         The width of the column for the name, it
 *                                             will only be used if the tag has a `name`
 *                                             property.
...
const renderTagInColumns = R.curry(
  (tagColumnWidth, typeColumnWidth, nameColumnWidth, descriptionColumnWidth, tag) => {
    ...
    const firstLineParts = [`@${tag.tag}`.padEnd(tagColumnWidth, ' ')];

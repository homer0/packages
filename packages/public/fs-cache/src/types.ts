 ...
 *
 * @template T The type of the value that will be stored. This is a generic because, by
 *   default, the service works with string, but there are custom entries, like for JSON,
 *   that use a different type.
 */
export type FsCacheEntryOptions<T = string> = {
   ...
   * returned. The idea of this option is for cases where you have to decide whether or
   * not to use the cache: you won't have to define the `init` function outside the call,
   * and then use an `if` to decide whether to call it or send it as a parameter.
   *
   * @default false
   */
  skip?: boolean;
  ...
  keepInMemory?: boolean;
  /**
   * This can be used to overwrite the extension the file should use. If is not specified,
   * the service option will be used.
   */
  extension?: string;
  /**
...
export type FsCacheCleanOptions = {
  /**
   * This function could be used to decide whether or not to actually remove a file. If
   * not specified, the default implementation will always return `true`.
   */
  shouldRemove?: FsCacheShouldRemoveFn;
  /**
   * This can be used to overwrite the extension the cache files should have. If is not
   * specified, the service option will be used.
   */
  extension?: string;
  /**
   * A TTL to use to validate whether the file is expired or not. If is not specified, the
   * service default will be used.
   */
  ttl?: number;
};

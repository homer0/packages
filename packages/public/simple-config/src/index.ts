...
export type SimpleConfigOptions = {
   ...
   * {@link SimpleConfig#load} to load it. The default value is created using `name`, plus
   * `.config.js`.
   *
   ...
   * {@link SimpleConfig.loadFromEnv}. The default value is created using the upper case
   * version of `name`, plus `_CONFIG`.
   *
   * @default 'APP_CONFIG'
   */
  envVarName?: string;
   ...
   * project root. The default value is generated by using the `name` option,
   * `config/[name]`.
   *
   * @default 'config/app'
   */
   ...
   * trying to load. The default value is created using the `name` option, and adding
   * `.[name].config.js` as suffix.
   *
...
export class SimpleConfig {
   ...
   * @returns When a name is specified, if the configuration doesn't exist, it will return
   *   `undefined`.
   */
  getConfig<T = unknown>(name: string = ''): T | undefined {
   ...
   * will overwrite the active configuration. This could also be used to create a new
   * configuration, by just sending a name for one that doesn't exist in the service.
   *
   * @template T The type of the configuration, for the return value.
   * @param config The new configuration settings.
   * @param name The name of the configuration.
   * @param mergeConfig Whether or not to merge the new settings with the existing ones.
   ...
   *
   * @example <caption>Get a single setting</caption>
   *   const value = config.get('some-setting');
   *
   * @example <caption>Get multiple settings</caption>
   *   const { settingOne, settingTwo } = config.get(['settingOne', 'settingTwo']);
   *
   * @example <caption>Get multiple settings as an array</caption>
   *   const [settingOne, settingTwo] = config.get(['settingOne', 'settingTwo'], true);
   *
   * @example <caption>Use paths</caption>
   ...
   * @param asArray When `setting` is an Array, if this is `true`, instead of returning a
   *   dictionary, it will return an array of settings.
   */
  ...
  /**
   * Sets the value of a setting or settings on the active configuration. If both the
   * current and the new value of a setting are objects, then instead of overwriting it,
   * the method will merge them.
   *
   * @example <caption>Set a single setting</caption>
   *   config.set('some-setting', 'some-setting-value');
   *
   * @example <caption>Set multiple settings</caption>
   *   config.set({
   *     settingOne: 'valueOne',
   *     settingTwo: 'valueTwo',
   *   });
   *
   * @param setting The name of the setting to update, or a dictionary of settings and
   *   their values.
   * @param value The value of the setting. This is only used when `setting` is a string.
   * @param mergeObjects If both the current and the new value of a setting are objects,
   *   then instead of overwriting it, the method will merge them.
   ...
   * @template T The type of the configuration, for the return value.
   * @param name The name of the configuration.
   * @param switchTo If the service should switch to the new configuration after loading
   *   it.
   * @param failWithError If this is true and the file doesn't exist, it throw an error.
   ...
   * @param switchTo If the service should switch to the new configuration after adding
   *   it.
   ...
   * name was provided in the constructor), and if there is, tries to load it from its
   * file (by calling {@link SimpleConfig.loadFromFile}).
   *
   * @template T The type of the configuration, for the return value.
   ...
   * @param force A way to force the service to switch, even if the `allowConfigSwitch`
   ...
   * @param config The configuration settings.
   * @param switchTo Whether or not to switch it to the active configuration after adding
   *   it.
   */

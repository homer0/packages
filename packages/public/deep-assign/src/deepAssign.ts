 ...
 * the {@link DeepAssign#assign} method to work.
 */
export class DeepAssign {
   ...
   * calling either {@link DeepAssign#mergeObjects} or {@link DeepAssign#mergeArrays}. If
   * the objects are not compatible, or `source` is not defined, it will return a copy of
   * `target`.
   *
   * @param source The base object.
   * @param target The object that will be merged in top of `source`.
   * @param ignoreArrayMode Whether or not to ignore the option that tells the class how
   *   array assignments should be handled. This parameter exists because, when called
   *   directly from {@link DeepAssign#assign}, it doesn't make sense to use a strategy
   *   different than 'merge'.
   */
   ...
   *   class how array assignments should be handled. This parameter exists because, when
   *   called directly from {@link DeepAssign#assign}, it doesn't make sense to use a
   *   strategy different than 'merge'. Default is `false`
   */
